...
class plxShow {
	...
	/**
	 * Méthode qui affiche le titre du book linké (variable $type='link') ou non
	 *
	...
	/**
	 * Méthode qui affiche le sous-titre du book
	 *
	 * @return	stdout
	 * @scope	global
	 * @author	Anthony GUÉRIN et Florent MONTHEL
	...
	**/
	 ...
	 * @param	menubook	position du menu Book (si non renseigné le menu n'est pas affiché)
	 * @return	stdout
	 * @scope	global
	 * @author	Stephane F
	 **/
	public function staticList($extra='', $format='<li id="#static_id" class="#static_class"><a href="#static_url" class="#static_status" title="#static_name">#static_name</a></li>', $format_group='<span class="#group_class">#group_name</span>', $menubook=false) {
		...
		if($menubook) {
			if($this->plxMotor->aConf['homestatic']!='' AND isset($this->plxMotor->aStats[$this->plxMotor->aConf['homestatic']])) {
				if($this->plxMotor->aStats[$this->plxMotor->aConf['homestatic']]['active']) {
					$menu = str_replace('#static_id','page-book',$format);
					if ($this->plxMotor->get AND preg_match('/^(book|categorie|archives|tag|article)/', $_SERVER['QUERY_STRING'])) {
						$menu = str_replace('#static_status','active',$menu);
					} else {
						$menu = str_replace('#static_status','noactive',$menu);
					}
					$menu = str_replace('#static_url', $this->plxMotor->urlRewrite('?book'),$menu);
					$menu = str_replace('#static_name',L_PAGEBOOK_TITLE,$menu);
					array_splice($menus, (intval($menubook)-1), 0, array($menu));
				}
			}
		}
		...
		# Affichage des pages statiques + menu Accueil et Réalisations
		if($menus) {
	...
	/**
	 * Méthode qui affiche un lien vers la page book.php
	 *
	 ...
	 * @author	Stephane F
	 **/
	public function pageBook($format='<li id="#page_id"><a class="#page_status" href="#page_url" title="#page_name">#page_name</a></li>') {
		# Hook Plugins
		if(eval($this->plxMotor->plxPlugins->callHook('plxShowPageBook'))) return;

		if($this->plxMotor->aConf['homestatic']!='' AND isset($this->plxMotor->aStats[$this->plxMotor->aConf['homestatic']])) {
			if($this->plxMotor->aStats[$this->plxMotor->aConf['homestatic']]['active']) {
				$name = str_replace('#page_id','page-book',$format);
				if ($this->plxMotor->get AND preg_match('/^(book|categorie|archives|tag|article)/', $_SERVER['QUERY_STRING'])) {
					$name = str_replace('#page_status','active',$name);
				} else {
					$name = str_replace('#page_status','noactive',$name);
				}
				$name = str_replace('#page_url', $this->plxMotor->urlRewrite('?book'),$name);
				$name = str_replace('#page_name',L_PAGEBOOK_TITLE,$name);
				echo $name;
			}
		}
	}
